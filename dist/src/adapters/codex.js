...
export const codexAdapter = { kind: 'codex', version: '1.0.0', supports: s => s.kind === 'codex' };
...
export function adaptCodex(source, parsed) {
    ...
    if (policy === 'never')
        return { ...base, recognized: true, envelopeValid: true, approvals: [...base.approvals, { principal: `${source.principalId}:${source.id}:V1`, applicability: 'potential' }] };
    ...
    if (policy === 'on-request' || policy === 'untrusted')
        return { ...base, recognized: true, envelopeValid: true };
    return { ...base, recognized: true, diagnostics: [...base.diagnostics, 'AF_CODEX_APPROVAL'] };
}
