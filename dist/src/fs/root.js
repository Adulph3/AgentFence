...
export async function checkedRoot(input) { if (!input || input.includes('\0') || /^(?:\\\\|\\\.\\)/.test(input) || /^[A-Za-z]:.*:/.test(input))
    ...
    if (component.isSymbolicLink())
        throw new Error('AF_ROOT_LINK');
} const st = await lstat(root); if (!st.isDirectory() || st.isSymbolicLink())
    throw new Error('AF_ROOT_INVALID'); const real = await realpath(root); if (real === parse(real).root || real === resolve(homedir()))
    throw new Error('AF_ROOT_REJECTED'); return real; }
