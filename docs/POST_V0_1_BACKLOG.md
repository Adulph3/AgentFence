...
Actual Node 24.21.0 Linux validation is complete. Accepted external evidence gaps remain: macOS and Windows runs; Linux denied-network syscall tracing; a wholly cache-empty consumer; broader dependency-audit evidence; actual Windows ACL validation; package ownership; a private security-reporting route; and publication authorization. These require external environments or owner decisions and did not conceal a confirmed local Critical/High defect in the final review.
