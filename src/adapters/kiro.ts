...
export const kiroAdapter:Adapter={kind:'kiro',version:'1.0.0',supports:s=>s.kind==='kiro'};
const object=(value:unknown):Record<string,unknown>|undefined=>value&&typeof value==='object'&&!Array.isArray(value)?value as Record<string,unknown>:undefined;
const disabled=(server:Record<string,unknown>)=>server.disabled===undefined?'potential':typeof server.disabled==='boolean'?(server.disabled?'inactive':'potential'):'invalid';
...
export function adaptKiro(source:SourceRecord,parsed:unknown,kiroCorrelationSecret?:string):McpFactsResult {
 ...
 if(!/(?:^|\/)\.kiro\/hooks\/[^/]+\.json$/.test(source.relativePath.replace(/\\/g,'/')))return base;
 const root=object(parsed), raw=root?.hooks;
 if(raw===undefined)return base;
 if(!Array.isArray(raw))return {...base,recognized:true,diagnostics:[...base.diagnostics,'AF_KIRO_HOOKS']};
 const values:string[]=[];let invalid=false;
 for(const value of raw){const item=object(value);if(!item||typeof item.command!=='string'||item.command===''){invalid=true;continue;}values.push(item.command);}
 const hooks=values.slice(0,64).map((command,index)=>({principal:`${source.principalId}:${source.id}:H${index+1}`,command,applicability:'potential' as const,eventOrdinal:index+1}));
