export interface PromptFact { readonly credential:boolean; readonly bypass:boolean; readonly exfil:boolean; readonly automation:'destructive'|'risky'|false; }
const negated=(clause:string)=>/(?:\bdo not\b|\bnever\b|\bavoid\b|\bmust not\b)/i.test(clause);
/**
 ...
 */
export function analyzeInstructions(text:string): PromptFact {
 let fenced=false;const clauses:string[]=[];
 for(const line of text.split('\n')){
  if(/^\s*(?:```|~~~)/.test(line)){fenced=!fenced;continue;}
  if(fenced||/^\s*>/.test(line))continue;
  for(const clause of line.split(/[.;]|\bbut\b/i))if(clause.trim()&&!negated(clause))clauses.push(clause);
 }
