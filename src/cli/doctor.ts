...
const adapterKinds=['codex','claude-code','cursor','kiro','vscode','generic-mcp'] as const;
const pass=(test:()=>boolean):boolean=>{try{return test();}catch{return false;}};
...
export interface DoctorServices {
 ...
 readonly overrides?:{readonly codexHome:boolean;readonly claudeConfigDir:boolean};
 readonly escapeSelfTest?:()=>boolean;
 readonly scoreSelfTest?:()=>boolean;
}
export interface DoctorRunResult { readonly output:string; readonly exitCode:0|2; }
/** Pure, fixed-surface health report: it deliberately performs no discovery or probes. */
export function doctorReport(services:DoctorServices={}):DoctorReport{
...
export function doctor(json:boolean):string{return runDoctor(json).output;}
