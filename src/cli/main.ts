...
const write=(stream:NodeJS.WriteStream,text:string,signal?:AbortSignal):Promise<void>=>writeInterruptibly(stream,text,signal);
const fail=async(json:boolean,code:string,outputRequested=false):Promise<number>=>{try{if(json&&!outputRequested)await write(process.stdout,jsonFailure(code));else await write(process.stderr,'AgentFence request failed\n');}catch{}return 2;};
...
if(parsed){
 if(parsed.command==='help'){try{await write(process.stdout,usage);}catch{process.exitCode=2;}}
  ...
  const controller=new AbortController(),onInterrupt=()=>controller.abort();process.once('SIGINT',onInterrupt);
  try{process.exitCode=await runScan(parsed.request!,parsed.json,false,{runtimeVersion:process.versions.node,scan:(request,signal)=>scanProject(request,{},signal),writeReport:text=>write(process.stdout,text),writeDiagnostic:text=>write(process.stderr,text),...(parsed.output?destinationWriter(parsed.output,(text,signal)=>write(process.stderr,text,signal)):{}),render:report=>terminalReport(report,!parsed.noColor&&Boolean(process.stdout.isTTY)&&!process.env.NO_COLOR)},controller.signal);}finally{process.removeListener('SIGINT',onInterrupt);}
 }
}
