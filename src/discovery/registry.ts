...
export interface Pattern { readonly path: RegExp; readonly kind?: AgentKind; readonly parseKind:'json'|'jsonc'|'toml'|'markdown'; }
export const registry:readonly Pattern[]=[
...
export function matchCandidate(relative:string,scope:Scope='project'):Pattern|undefined{return registry.find(p=>!(scope==='project'&&/(?:^|\/)\.claude\.json$/.test(relative))&&p.path.test(relative));}
