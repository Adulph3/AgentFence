import type { Finding, Location, Severity } from '../core/types.js';
...
const hidden=(value:number,index:number)=>((value>=0&&value<32&&value!==9&&value!==10&&value!==13)||value===127||([0x200b,0x200c,0x200d,0xfeff].includes(value)&&index!==0));
const relevantControl=/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F\u200B-\u200D\u202A-\u202E\u2066-\u2069\uFEFF]/;

export interface UnicodeFindingsResult { readonly findings:readonly Finding[]; readonly truncated:boolean; }
/** Coordinates are 1-based UTF-16 columns with end-exclusive locations. */
export function boundedUnicodeFindings(text:string, location:Location, limit:number=LIMITS.findings):UnicodeFindingsResult {
 if(!relevantControl.test(text))return{findings:[],truncated:false};
 const findings:Finding[]=[];let line=1,column=1,index=0,truncated=false;
 for(const character of text){
  ...
  if(detectedRule){
   if(findings.length>=limit){truncated=true;break;}
    ...
    const severity:Severity=detectedRule==='AF-UNICODE-001'?'medium':value===0x200c||value===0x200d?'info':'low';
  ...
  if(character==='\n'){line++;column=1;}else column=endColumn;
  index+=character.length;
 }
 return{findings,truncated};
}
/** Legacy array convenience for isolated Unicode callers; bounded by scanner limits. */
export const unicodeFindings=(text:string,location:Location):Finding[]=>[...boundedUnicodeFindings(text,location).findings];
