...
test('injected doctor supports Node 22 and 24 with the same fixed metadata',()=>{
 for(const runtimeVersion of ['22.0.0','22.23.2','24.0.0','24.21.0']){const result=runDoctor(true,{...supported,runtimeVersion}),value=JSON.parse(result.output);assert.equal(result.exitCode,0);assert.equal(value.runtimeSupported,true);assert.equal(value.engineVersion,'0.2.0');assert.equal(validate(value),true,JSON.stringify(validate.errors));}
});
